...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
  ...
  jclass local_expander_ret_class = env->FindClass(
      "org/apache/arrow/gandiva/evaluator/VectorExpander$ExpandResult");
...
Status make_record_batch_with_buf_addrs(
    SchemaPtr schema, int num_rows, jlong *in_buf_addrs, jlong *in_buf_sizes,
    int in_bufs_len, std::shared_ptr<arrow::RecordBatch> *batch) {
...
void releaseProjectorInput(jbyteArray schema_arr, jbyte *schema_bytes,
                           jbyteArray exprs_arr, jbyte *exprs_bytes,
                           JNIEnv *env) {
...
Java_org_apache_arrow_gandiva_evaluator_JniWrapper_buildProjector(
  ...
  std::shared_ptr<Configuration> config =
      ConfigHolder::MapLookup(configuration_id);
  std::stringstream ss;
  ...
  if (!ParseProtobuf(reinterpret_cast<uint8_t *>(schema_bytes), schema_len,
                     &schema)) {
...
public:
  JavaResizableBuffer(JNIEnv *env, jobject jexpander, int32_t vector_idx,
      ...
      : ResizableBuffer(buffer, len), env_(env), jexpander_(jexpander),
...
Java_org_apache_arrow_gandiva_evaluator_JniWrapper_evaluateProjector(
    JNIEnv *env, jobject object, jobject jexpander, jlong module_id,
    jint num_rows, jlongArray buf_addrs, jlongArray buf_sizes,
    jint sel_vec_type, jint sel_vec_rows, jlong sel_vec_addr,
  ...
  if (in_bufs_len != env->GetArrayLength(buf_sizes)) {
    env->ThrowNew(gandiva_exception_,
                  "mismatch in arraylen of buf_addrs and buf_sizes");
    return;
  }
  ...
  do {
    ...
    status = make_record_batch_with_buf_addrs(holder->schema(), num_rows,
                                              in_buf_addrs, in_buf_sizes,
    ...
    for (FieldPtr field : ret_types) {
      ...
      if (arrow::is_binary_like(field->type()->id())) {
        if (jexpander == nullptr) {
          status = Status::Invalid("expression has variable len output "
                                   "columns, but the expander object is "
                                   "null");
          break;
        }
      ...
      auto array_data =
          arrow::ArrayData::Make(field->type(), output_row_count, buffers);
...
JNIEXPORT void JNICALL
Java_org_apache_arrow_gandiva_evaluator_JniWrapper_closeProjector(
...
void releaseFilterInput(jbyteArray schema_arr, jbyte *schema_bytes,
                        jbyteArray condition_arr, jbyte *condition_bytes,
                        JNIEnv *env) {
  env->ReleaseByteArrayElements(schema_arr, schema_bytes, JNI_ABORT);
...
JNIEXPORT jlong JNICALL
Java_org_apache_arrow_gandiva_evaluator_JniWrapper_buildFilter(
  ...
  if (!ParseProtobuf(reinterpret_cast<uint8_t *>(condition_bytes),
  ...
  // store the result in a map
  holder = std::shared_ptr<FilterHolder>(
      new FilterHolder(schema_ptr, std::move(filter)));
...
Java_org_apache_arrow_gandiva_evaluator_JniWrapper_evaluateFilter(
    JNIEnv *env, jobject cls, jlong module_id, jint num_rows,
    jlongArray buf_addrs, jlongArray buf_sizes, jint jselection_vector_type,
    jlong out_buf_addr, jlong out_buf_size) {
  gandiva::Status status;
  ...
  do {
    ...
    switch (selection_vector_type) {
    case types::SV_INT16:
      status = gandiva::SelectionVector::MakeInt16(num_rows, out_buffer,
                                                   &selection_vector);
      break;
    case types::SV_INT32:
      status = gandiva::SelectionVector::MakeInt32(num_rows, out_buffer,
                                                   &selection_vector);
      break;
    default:
...
JNIEXPORT void JNICALL
Java_org_apache_arrow_gandiva_evaluator_JniWrapper_closeFilter(
    JNIEnv *env, jobject cls, jlong module_id) {
